...
(define (ac-macro? fn env)
  (if (and (symbol? fn)
           ;; let a lexical variable override a macro
           (not (lex? fn env)))
...
(xdef ar-parameter make-parameter)

(xdef ar-parameterize
  (lambda (param val thunk)
...
(xdef tagged (lambda (x) (tnil (ar-tagged? x))))
